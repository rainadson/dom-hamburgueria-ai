...
export default function ManualOrder(){
 ...
 const defaultDeliveryRules=[{max_km:4,fee:4},{max_km:8,fee:6},{max_km:12,fee:9}];
 const [paymentMethods,setPaymentMethods]=useState<string[]>(['DINHEIRO','MULTIBANCO']);const [deliveryRules,setDeliveryRules]=useState(defaultDeliveryRules);
 const [form,setForm]=useState({customer_name:'',customer_phone:'',delivery_type:'PICKUP',address:'',delivery_distance_km:'',payment_method:'MULTIBANCO',amount_paid:''});
 const [lines,setLines]=useState<Line[]>([emptyLine()]);const [error,setError]=useState('');const [busy,setBusy]=useState(false);
 const [preview,setPreview]=useState<{customer_name:string;customer_phone:string;delivery_type:string;address:string|null;delivery_distance_km:number|null;delivery_fee:number;payment_method:string;total:number;change:number;items:{product:string;quantity:number;subtotal:number;components?:string[]}[]}|null>(null);
 ...
 <label>Nome<input required maxLength={150} value={form.customer_name} onChange={e=>field('customer_name',e.target.value)}/></label><label>Telefone{selectedCustomer?<span className="selected-customer-phone"><input required type="tel" value={selectedCustomer.phone} readOnly/><button type="button" onClick={()=>{setForm(current=>({...current,customer_phone:selectedCustomer.phone}));setSelectedCustomer(null);}}>Editar</button></span>:<input required type="tel" maxLength={20} value={form.customer_phone} onChange={e=>field('customer_phone',e.target.value)}/>}</label></fieldset>
 ...
 <fieldset disabled={busy||!!pending||!!sentId||!pendingReady}><legend>Entrega e pagamento</legend><label>Receção<select value={form.delivery_type} onChange={e=>field('delivery_type',e.target.value)}><option value="PICKUP">Levantamento</option><option value="DELIVERY">Entrega</option></select></label>{form.delivery_type==='DELIVERY'&&<><label>Morada<textarea required maxLength={500} value={form.address} onChange={e=>field('address',e.target.value)}/></label><label>Distância da entrega<select required value={form.delivery_distance_km} onChange={e=>field('delivery_distance_km',e.target.value)}><option value="">Escolha a distância</option>{deliveryRules.map((rule,index)=><option key={rule.max_km} value={rule.max_km}>{index===0?'Até':`Mais de ${deliveryRules[index-1].max_km} até`} {rule.max_km} km — € {Number(rule.fee).toFixed(2)}</option>)}</select></label></>}

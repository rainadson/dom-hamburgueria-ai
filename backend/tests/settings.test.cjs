...
test('LOJA reads settings while only ADMIN can update its own store',async()=>{
 ...
 const server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));const url=`http://127.0.0.1:${server.address().port}/settings`;const payload={restaurant_name:'Dom Hamburgueria',payment_methods:['DINHEIRO','MULTIBANCO']};

...
test('unexpected failures and malformed/oversized bodies return safe JSON errors',async()=>{
 const express=require('express');const app=express();app.use(express.json({limit:100}));app.post('/input',(_req,res)=>res.json({ok:true}));app.get('/error',async()=>{throw Error('PRIVATE_INTERNAL_DETAIL');});app.use(handleApiError);
 const server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));const base=`http://127.0.0.1:${server.address().port}`;
 try{for(const [path,options,status] of [['/error',{},500],['/input',{method:'POST',headers:{'content-type':'application/json'},body:'{"PRIVATE_BODY":'},400],['/input',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text:'x'.repeat(200)})},413]]){const response=await fetch(base+path,options);assert.equal(response.status,status);assert.match(response.headers.get('content-type'),/json/);assert.doesNotMatch(await response.text(),/PRIVATE|stack|Error:/);}}finally{server.closeAllConnections();await new Promise(r=>server.close(r));}
});
